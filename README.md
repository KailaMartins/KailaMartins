# Olá! 👋

Sou **Kaila Martins**.

Transformando requisitos em soluções de software com foco em qualidade, organização e aprendizado contínuo.

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Oracle](https://img.shields.io/badge/Oracle-F80000?style=for-the-badge&logo=oracle)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git)

💼 Atuo como **Analista de Sistemas**, com experiência em análise de requisitos, banco de dados e qualidade de software.

🎓 Graduanda em Sistemas de Informação e apaixonada por tecnologia, sempre buscando aprender novas ferramentas e desenvolver soluções eficientes.

---

## 🚀 Sobre mim

- 💼 Analista de Sistemas
- 🧪 Atuação com Testes e Qualidade de Software 
- 📚 Aprofundando meus conhecimentos em Python e Banco de Dados
- 💡 Interesse em Automação de testes e Engenharia de Software
- 🌱 Aprendizado contínuo e evolução profissional

---

## 💻 Tecnologias

### Linguagens

- Python (em aprendizado)
- SQL

### Banco de Dados

- Oracle
- PostgreSQL
- MySQL
- MariaDB

### Ferramentas

- Git
- Docker
- Kubernetes
- Oracle SQL Developer
- Power BI
- FastAPI

---

## 📖 Atualmente estudando

- 🐍 Python
- 🗄️ Banco de Dados (Oracle e PostgreSQL)
- 🧪 Testes de Software
- ⚙️ Automação de Processos

---

## 🎯 Objetivos

Busco aprimorar meus conhecimentos em desenvolvimento de software, banco de dados e testes de qualidade, contribuindo para a criação de sistemas confiáveis, eficientes e de alta qualidade.

---

## 📫 Contato

- LinkedIn: https://www.linkedin.com/in/kaila-martins-67424622a
- E-mail:kailageovana@gmail.com


## 📊 Estatísticas

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=KailaMartins&show_icons=true&theme=tokyonight)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=KailaMartins&layout=compact&theme=tokyonight)
